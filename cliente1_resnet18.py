...
# Função para salvar pesos do modelo
def save_weights(model, filename):
    os.makedirs("weights", exist_ok=True)
    filepath = os.path.join("weights", filename)
    torch.save(model.state_dict(), filepath)

# Modelo ResNet18 com cabeça personalizada
def create_model():
...
class PneumoniaClient(fl.client.NumPyClient):
    def __init__(self):
        ...
        self.learn = Learner(
            ...
            metrics=accuracy,
            cbs=SaveCallback(save_path="training_logs"),
        )
    ...
    def fit(self, parameters, config):
        ...
        self.learn.fine_tune(6)  # Define 6 épocas
